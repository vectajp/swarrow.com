...
- 作成日: 2026-07-11
- 改訂日: 2026-07-11
- ステータス: Approved（回答品質・個別最適メッセージ改訂を2026-07-11承認）
- Issue: none
...
- Hero の `そのまち` は、市区町村の担当者に自分たちのための回答だと
  感じてもらうブランド表現として使用する。
...
| 1 | `top` | Hero | 回答精度に妥協せず、そのまちの情報と業務に合う答えを磨き続ける約束を提示 | `hero-city.webm`、MP4、poster |
...
- **When** 最初の画面を確認する
- **Then** 回答精度に妥協せず、知識、回答ルール、参照元を個別に調整し、
  そのまちの答えを公開後も磨き続けるサービスだと理解できる
...
| 市区町村に合う回答を継続的に磨く姿勢を示す | Hero message、Support copy | AC1、AC13、AC14 |
...
| `そのまちの答えを、磨き続ける。` と表現する | 100% | 市区町村の担当者に自分たちのための回答だと感じてもらうため |
