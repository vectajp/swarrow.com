...
- 作成日: 2026-07-11
- 改訂日: 2026-07-11
- ステータス: Approved（回答品質メッセージ改訂を2026-07-11承認）
- Issue: none
...
「電話とホームページの定型対応をAIに。職員は、本来の仕事へ。」と
業務便益を伝えているが、次の決定的なメッセージ課題が残っている。
...
「根拠を確認できる回答範囲だけを、検証してから公開する」へ変更する。
...
| 1 | `top` | Hero | 根拠を確認できる回答範囲だけを、事前検証してから公開する約束を提示 | `hero-city.webm`、MP4、poster |
...
- H1 1行目: `根拠を確認できる回答だけを、住民へ。`
- H1 2行目: `検証してから、公開する。`
- Support copy:
  `Swarrow ChatとSwarrow Callは、自治体の公式情報をもとにした回答を
  公開前に評価します。Vectaが回答方針・参照元・不足情報を整え、自治体と
  確認した範囲から公開。公開後も利用状況と低評価質問を定期的に確認し、
  回答品質を継続的に改善します。`
- Primary CTA: `導入相談・デモを依頼する`
- Secondary CTA: `回答品質の仕組みを見る` → `#quality`

ニュースカードは Hero から外し、品質の約束、製品名、CTA、既存 Hero 動画だけに
焦点を絞る。H1 の「回答だけ」は全回答の正確性保証ではなく、公開前に
根拠と内容を確認できた回答範囲だけを住民向けに公開する運用上の約束を示す。
...
- **When** 最初の画面を確認する
- **Then** 自治体の公式情報をもとにした回答を公開前に評価し、根拠と内容を
  確認できた範囲から公開するサービスだと理解できる
